...
def login():
    ...
    # For simplicity, we'll use a basic username/password check here
    if username == 'admin' and password == 'admin123':
        # Store login data in the login table
        login_data = LoginData(username=username, login_time=datetime.now())
...
class PDFChatBot:
    ...
    def __init__(self,db_faiss_path=None):
        ...
        # self.db_faiss_path = 'vectordb/db_faiss_Ticket_1'
        # self.db_faiss_path='vectordb/db_faiss_Imran Bashir - Blockchain Consensus_ An Introduction to Classical, Blockchain, and Quantum Consensus Protocols-Apress (2022)'
        # self.db_faiss_path='vectordb/db_faiss_Advanced Cardiovascular Disease Prediction'
        # self.db_faiss_path='vectordb/db_faiss_Return'

        self.db_faiss_path='vectordb/db_faiss_ml_project_proposal_ayantanu_laha_rajarshi_saha'
...
def search_history():
    ...
    # Perform a search in the history table
    connection = create_connection()
    search_results = []
    ...
    if connection:
        try:
            cursor = connection.cursor()
            sql_query = "SELECT query, answer FROM history WHERE query LIKE %s"
            cursor.execute(sql_query, ('%' + search_query + '%',))
            result = cursor.fetchall()
            search_results = [{'query': row[0], 'answer': row[1]} for row in result]
        except Error as e:
            print(f"Error searching history from MySQL database: {e}")
        finally:
            if connection.is_connected():
                cursor.close()
                connection.close()
